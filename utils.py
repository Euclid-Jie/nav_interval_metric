...
def generate_trading_date(
    ...
    working_date = pd.date_range(begin_date, end_date, freq="B").values.astype("datetime64[D]")
    ...
    trading_date_df["trading_date"] = np.where(is_trading, working_date, np.datetime64("NaT"))
    trading_date_df["trading_date"] = pd.to_datetime(trading_date_df["trading_date"]).ffill()
